...
class ControlNode(Node):
    def __init__(self):
        ...
        self.robotB_progress = ["로봇 대기", "환자 위치로 이동", "구급대원 위치로 이동", "구급대원 대기", "구급대원과 함께 사고 지역으로 이동", "도킹 스테이션 복귀"]
        ...
        self.robot3_current_pose.pose.position.x = 0.0
        self.robot3_current_pose.pose.position.y = 0.0

        self.robot5_current_pose = PoseStamped()
        self.robot5_current_pose.header.frame_id = "map"
        self.robot5_current_pose.pose.position.x = -0.00147931
        self.robot5_current_pose.pose.position.y = -4.19319
        ...
        self.emt_status_sub = self.create_subscription(Bool, '/emt_arrival_status', self.emt_arrival_callback, 10)
    ...
    # [추가됨] 구급대원 도착 상태 수신 콜백
    def emt_arrival_callback(self, msg):
        if msg.data:
            self.stop = True
            self.get_logger().warn("!!! EMT ARRIVAL SIGNAL RECEIVED -> ACTIVATING STOP !!!")

    # 메인 로직 타이머 (역할 배정 및 상태 모니터링)
    def timer_callback(self):
        # 비상 정지 활성화 시 처리 로직
        if self.stop:
            # [안전장치] 방금 막 정지 신호가 발생했는지 확인하는 플래그
            ...
            if not self.stop_published:
                ...
                just_triggered = True # [중요] 방금 발행했음을 표시
                self.get_logger().info("Emergency Stop & Return Message Published.")
            
            self.publish_count = 0

            # 정지 후 로봇들이 모두 복귀하여 대기 상태(0)가 되면 정지 상태 자동 해제
            # [중요] 방금 정지 신호를 발행한 루프(just_triggered)라면 즉시 해제하지 않음
            if not just_triggered and self.robotA_raw_status == 0 and self.robotB_raw_status == 0:
                self.stop = False
                self.stop_published = False 
