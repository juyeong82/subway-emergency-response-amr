...
DOCK_APPROACH_POSITION_ROBOT5 = (-0.5, -3.6, 90.0)
...
class RescueRobotEventBased(Node):
    def __init__(self):
        ...
        self.get_logger().info("✅ RescueRobot 초기화 완료 (대기 상태: /robot_role=False 대기 중)")
    ...
    def robot_select_callback(self, msg: Bool):
        """로봇 선택 및 활성화 이벤트 (/robot_role)"""
        # 이미 활성화되었거나 미션 시작 중이면 무시
        if self.is_activated or self.mission_started:
            return
        
        # [수정] msg.data가 False일 때만 robot3로 설정하고 활성화
        if msg.data is False:
            self.robot_selected = 'robot3'
            self.is_activated = True  # 시스템 활성화
            self.get_logger().info(f"🤖 역할 부여됨(False): {self.robot_selected} 활성화 완료!")
            self.try_start_mission()
        else:
            self.get_logger().info("⏳ /robot_role: True 수신 (대기 중... False를 기다립니다)")

    def goal_callback(self, msg: PoseStamped):
        """목표 좌표 수신 이벤트"""
        # [가드] 활성화되지 않았으면 무시
        if not self.is_activated:
            # self.get_logger().info("⚠️ 활성화 전이므로 목표 좌표 무시")
            return
    ...
    def try_start_mission(self):
        """활성화됨 + 로봇 선택 + 목표 좌표가 모두 있으면 미션 시작"""
        if self.is_activated and not self.mission_started and self.robot_selected and self.target_coordinate:
            self.mission_started = True
            self.get_logger().info("✅ 미션 조건 충족! 언도킹 시작")
    ...
    def reset_mission(self):
        ...
        self.get_logger().info("🔄 미션 리셋... (대기 모드로 전환)")
        self.stop_beep()
