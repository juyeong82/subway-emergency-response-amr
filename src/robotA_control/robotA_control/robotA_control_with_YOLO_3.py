...
class AEDNavigatorOctagonSmooth(Node):
    def __init__(self):
        ...
        self.is_activated = False  # [추가] 로봇 활성화 상태 플래그 (True일 때만 동작)
        
        self.trigger_lock = threading.Lock()
        self.crowd_lock = threading.Lock()
        self.aed_lock = threading.Lock()
        self.stop_lock = threading.Lock()

        # --- [Subscribers] ---
        # Trigger Subscriber는 항상 열려 있어야 함
        ...
        self.get_logger().info('=== AED Octagon Smooth Patrol Ready (Wait for /robot_role=True) ===')
    ...
    def patient_dist_callback(self, msg):
        if not self.is_activated: return # 활성화 전이면 무시
        ...
        try:
            if msg.data is True:
                ...
                if self.selected_robot_ns == '/robot3':
                    ...
                    self.get_logger().error("로봇 네임스페이스가 설정되지 않아 복귀할 수 없습니다.")
                    return
                
                home_pose.pose.orientation = self.euler_to_quaternion(target_yaw)
                
                self.get_logger().info(f"복귀 시작: {self.selected_robot_ns} -> 좌표({home_pose.pose.position.x}, {home_pose.pose.position.y})")
    ...
    def trigger_callback(self, msg):
        # Trigger Lock은 중복 실행 방지를 위함
        if not self.trigger_lock.acquire(blocking=False):
            return

        try:
            # [수정] True가 아니면 로봇을 활성화하지 않고 대기
            if not msg.data:
                self.get_logger().info(">> /robot_role: False 수신. 활성화 대기 중...")
                return
            ...
            # [수정] msg.data가 True일 때만 /robot3로 설정
            target_ns = '/robot3'
            ...
            self.get_logger().info(f'>> 로봇 선택됨: {target_ns} (시스템 활성화 완료)')
            
            self.is_initialized = True
            self.is_setup_in_progress = False
            self.is_activated = True # [중요] 시스템 활성화
        finally:
    ...
    def perform_undock_sequence(self):
        ...
        try:
            ...
            self.get_logger().info(">> Undock 완료 (추가 이동 없음).")
            return True
    ...
    def perform_docking(self):
        self.get_logger().info('>> Docking Action 요청 중...')
    ...
    def dock_response_callback(self, future):
        ...
        if not goal_handle.accepted: 
            ...
            return
        self.get_logger().info('>> Docking Goal 수락됨. 도킹 진행 중...')
    ...
    def patient_pose_callback(self, msg):
        # [Guard] 로봇이 활성화되지 않았다면 명령 무시
        if not self.is_activated:
            self.get_logger().info(">> 명령 무시: 로봇이 아직 활성화(/robot_role=True)되지 않았습니다.")
            return

        # 1. 미션 상태 체크 (이미 실행 중이면 즉시 리턴)
        ...
        if self.is_setup_in_progress:
            self.get_logger().warn(">> 로봇 설정 중입니다. 잠시 후 다시 시도해주세요.")
            return

        if not self.selected_robot_ns or self.map_data is None: 
            return

        # 3. 요청 처리 중 플래그 체크
        if self.is_processing_patient_req:
            self.get_logger().info(">> 이전 환자 요청을 처리(Undock 등) 중입니다. 중복 요청 무시.")
            return

        # 4. 플래그 설정
        self.is_processing_patient_req = True
        
        self.get_logger().info(">> 새로운 환자 요청 접수. 처리 시작.")
    ...
    def process_patient_mission_thread(self, msg):
        try:
            # Undock 수행
            if self.is_docked:
                self.get_logger().info(">> 환자 발생 수신: 도킹 해제(Undock) 시퀀스 먼저 시작.")
                success = self.perform_undock_sequence()
                if not success:
                    self.get_logger().error(">> Undock 실패로 인해 환자 미션을 시작하지 못했습니다.")
                    return 
                time.sleep(1.0)

            if self.robot_current_pose is None:
                self.get_logger().warn(">> 로봇 위치를 알 수 없어 환자 접근을 시작할 수 없습니다.")
                return
    ...
    def aed_detected_callback(self, msg):
        ...
        if not self.aed_lock.acquire(blocking=False):
            return
    ...
    def crowd_callback(self, msg):
        ...
        if self.mission_state == 'PATROLLING': return
        
        if not self.crowd_lock.acquire(blocking=False):
            return
